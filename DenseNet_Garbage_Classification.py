...
pretrained_model3 = tensorflow.keras.applications.DenseNet201(input_shape=(224,224,3),include_top=False,weights='imagenet',pooling='avg')
pretrained_model3.trainable = False

inputs3 = pretrained_model3.input
x3 = tensorflow.keras.layers.Dense(128, activation='relu')(pretrained_model3.output)
outputs3 = tensorflow.keras.layers.Dense(6, activation='softmax')(x3)
model = tensorflow.keras.Model(inputs=inputs3, outputs=outputs3)

model.compile(optimizer='adam',loss='categorical_crossentropy',metrics=['acc'])

# Train the model
#es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=10)
mc = ModelCheckpoint('DenseNet201_best_weights.h5', monitor='val_acc', mode='max', verbose=1, save_best_only=True)
...
model.save("DenseNet_Best_Model.h5")
...
plt.legend()
plt.savefig('DenseNet_train_val_acc.png')
plt.figure()
...
plt.legend()
plt.savefig('DenseNet_train_val_loss.png')
plt.show()
